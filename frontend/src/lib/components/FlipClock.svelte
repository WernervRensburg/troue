  ...
  onMount(
    () =>
      (interval = setInterval(() => {
        ...
        if (showMinutes)
          newData.push(myDate.getMinutes().toString().padStart(2, "0"));
        if (showSeconds)
          newData.push(myDate.getSeconds().toString().padStart(2, "0"));
...
<div
  class="clock"
  style:width="calc((2ch + 0.5em) * {segmentsCount} + 0.1em * {segmentsCount -
    1})"
